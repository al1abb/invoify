/**
 * Render invoice Template 2 using Forme and save as PDF + PNG.
 *
 * Usage: node scripts/render-forme-t2.mjs
 */
...
const data = {
  ...
  details: {
    ...
    currency: 'USD',
    language: 'English',
    taxDetails: { amount: 15, taxID: 'TAX-123', amountType: 'percentage' },
...
function InvoiceTemplate2(data) {
  const { sender, receiver, details } = data;

  return React.createElement(Document, { title: `Invoice ${details.invoiceNumber}` },
        ...
        React.createElement(View, { style: { width: 350, ...tw('gap-[7px]') } },
          React.createElement(View, { style: tw('flex-row') },
            React.createElement(View, { style: { flex: 4 } },
              React.createElement(Text, { style: tw('font-semibold text-gray-800 text-right mr-6') }, 'Subtotal:')),
            React.createElement(View, { style: { flex: 4 } },
          ...
          details.discountDetails?.amount != undefined && details.discountDetails?.amount > 0 &&
            React.createElement(View, { style: tw('flex-row') },
              React.createElement(View, { style: { flex: 4 } },
                React.createElement(Text, { style: tw('font-semibold text-gray-800 text-right mr-6') }, 'Discount:')),
              React.createElement(View, { style: { flex: 4 } },
          ...
          details.taxDetails?.amount != undefined && details.taxDetails?.amount > 0 &&
            React.createElement(View, { style: tw('flex-row') },
              React.createElement(View, { style: { flex: 4 } },
                React.createElement(Text, { style: tw('font-semibold text-gray-800 text-right mr-6') }, 'Tax:')),
              React.createElement(View, { style: { flex: 4 } },
                React.createElement(Text, { style: tw('text-gray-500 text-right') },
          ...
          details.shippingDetails?.cost != undefined && details.shippingDetails?.cost > 0 &&
            React.createElement(View, { style: tw('flex-row') },
              React.createElement(View, { style: { flex: 4 } },
                React.createElement(Text, { style: tw('font-semibold text-gray-800 text-right mr-6') }, 'Shipping:')),
              React.createElement(View, { style: { flex: 4 } },
                React.createElement(Text, { style: tw('text-gray-500 text-right') },
          ...
          React.createElement(View, { style: tw('flex-row') },
            React.createElement(View, { style: { flex: 4 } },
              React.createElement(Text, { style: tw('font-semibold text-gray-800 text-right mr-6') }, 'Total:')),
            React.createElement(View, { style: { flex: 4 } },
          ...
          details.totalAmountInWords &&
            React.createElement(View, { style: tw('flex-row') },
              React.createElement(View, { style: { flex: 4 } },
                React.createElement(Text, { style: tw('font-semibold text-gray-800 text-right mr-6') }, 'Total in words:')),
              React.createElement(View, { style: { flex: 4 } },
...
// Render
console.log('Rendering Forme PDF (Template 2)...');
const pdfBytes = await renderDocument(InvoiceTemplate2(data));

const outDir = resolve(import.meta.dirname, '..', '.github');
writeFileSync(resolve(outDir, 'template-2-after.pdf'), pdfBytes);
console.log(`Wrote ${pdfBytes.length} bytes to .github/template-2-after.pdf`);

// Convert to PNG
import { execSync } from 'node:child_process';
const pdfPath = resolve(outDir, 'template-2-after.pdf');
const pngBase = resolve(outDir, 'template-2-after');
execSync(`pdftoppm -png -r 150 -singlefile "${pdfPath}" "${pngBase}"`);
console.log(`Wrote .github/template-2-after.png`);
