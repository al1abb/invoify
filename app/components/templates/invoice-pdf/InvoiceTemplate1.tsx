...
const InvoiceTemplate = (data: InvoiceType) => {
  ...
  return (
    <InvoiceLayout data={data}>
      ...
      <div className="mt-6 mb-4 flex justify-between font-medium text-left">
        <div className="flex flex-col">
          <dl className="gap-x-3">
            {sender.customInputs?.map((input) => (
              <dd key={input.key} className="text-md text-gray-600 ml-2">
                {input.value}
              </dd>
            ))}
          </dl>
        ...
        <div className="flex flex-col text-right">
          <dl className="gap-x-3">
            {receiver.customInputs?.map((input) => (
              <dd key={input.key} className="text-md text-gray-600 ml-2">
                {input.value}
              </dd>
            ))}
